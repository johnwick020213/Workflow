...
void CloudiskServer::loadUserRegisterModule()
{
    _httpserver.POST("/user/signup", [](const HttpReq *req, HttpResp * resp, SeriesWork * series){
        if(req->content_type() == APPLICATION_URLENCODED) {
            ...
            // 创建一个服务的代理 client
            UserService::SRPCClient client("127.0.0.1",1412);
            // req 是rpc的请求
            ReqSignup req;
            req.set_username(username);
            req.set_password(password);
            // 创建一个rpc任务，并且根据任务的结果决定给网页响应的内容
            auto rpcTask = client.create_Signup_task([resp](RespSignup *respRpc, srpc::RPCContext *ctx){
                int code = respRpc->code();
                if(code == 100){
                    resp->String("SUCCESS");
                }
                else{
                    resp->String("Signup fail");
                }
            });
            rpcTask->serialize_input(&req);
